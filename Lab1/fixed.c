...
void ST7735_uBinOut8(uint32_t n) {
	if(outOfBinRange(n)) {
		...
		uint32_t frac_part = ((n & 0x000000FF) * 100) / 256;
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], int32_t bufY[]) {
	...
	while(index < num) {
		...
		uint32_t xLCD = width*(bufX[index] - minX_global)/(maxX_global - minX_global);
		uint32_t yLCD = height - height*(bufY[index] - minY_global)/(maxY_global - minY_global);
